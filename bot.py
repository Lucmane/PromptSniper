...
def save_users(users): with open(USER_DB_PATH, 'w') as f: json.dump(list(users), f)
...
async def check_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() user_id = query.from_user.id
